...
function safe_filter($string, $flags = null) {
    if(is_array($string)) {
        ...
        if($flags === null) {
            //sql字符串换
            $replace = ['insert ','add ','select ','sleep ','union ','create ','delete ','drop ','or ','passwd ','password','modify ','alter ','join ','eval ','sysopen ','system ','where ','update ','like ','rename ','cas ','convert','md5','array','root ','etc','==','@','!=','&'];
 ...
 * 获取门店信息
 */
function getStoreInfo($filter, $fields = '*')
{
