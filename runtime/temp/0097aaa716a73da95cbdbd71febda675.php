<?php /*a:2:{s:69:"E:\wwwroot\2025\git\baseShop\application\manage\view\goods\index.html";i:1735887385;s:64:"E:\wwwroot\2025\git\baseShop\application\manage\view\layout.html";i:1735887385;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <script>
        ...
        var Jshop_Image = "<?php echo url('manage/images/uploadImage'); ?>";
        var manage_Image = "<?php echo url('manage/images/manage'); ?>";
    </script>
...
<form class="layui-form seller-form"  action="" >
    <div class="layui-form-item">
		...
		<div class="layui-inline">
			...
			<div class="layui-input-inline seller-inline-2">
	            <select name="open_point" id="open_point">
	                <option value="">全部</option>
	                <option value="1">积分+现金</option>
	                <option value="2">积分</option>
	                <option value="3">未开启积分</option>
	            </select>
	        </div>
		</div>
        <div class="layui-inline">
        ...
        <div class="layui-inline">
            ...
            <div class="layui-input-inline seller-inline-2">
                <select name="batchOperate" id="batchOperate" lay-filter="batchOperate">
                    ...
                    <?php if(!get_addons_status('StockControl')): ?>
...
<div id="importGoods" style="display: none;">
    <form class="layui-form import-form" method="post" action="<?php echo url('ietask/import'); ?>" enctype="multipart/form-data">
        <div class="layui-form-item">
            <div class="layui-margin-10">
                <blockquote class="layui-elem-quote layui-text">
                    请先下载<a href="<?php echo url('ietask/importTemplete',['tplName'=>'goods']); ?>">导入模板</a><br/>
                    商品导入，请务必填写商品编码。
                </blockquote>
            </div>
            <div class="layui-inline">
...
<script>
    ...
    layui.use(['form', 'layedit','upload', 'laydate','table','element'], function(){
        ...
        table = layui.table.render({
            ...
            cols: [[
                ...
                {field:'operating',width:300,title:'操作', align: 'center',templet:function(data){
                        var html = '';
                        ...
                        html += '<a class="layui-btn layui-btn-xs comment-class" href="<?php echo url('user/comment'); ?>?goods_id=' + data.id + '">查看评价</a>';
                        html += '<a class="layui-btn layui-btn-danger layui-btn-xs preview-class" data-id="' + data.id + '" data-point-type="'+data.open_point+'">预览详情</a>';
                        return html;
                    }},
                ...
                {field:'stock', width:100,title:'库存', align: 'center',templet:function(data){
                    var html = '<a href="javascript:void(0);" title="'+data.stock+'">' + data.stock + '</a>';
                        return html;
                }},
                ...
                {field:'point', minWidth:100,title:'积分', align: 'center'},
        ...
        // 上下架操作
        layui.form.on('switch(is_able)', function (obj) {
            JsPost("<?php echo url('Goods/changeState'); ?>", {id: this.value, type: 'able', state: obj.elem.checked}, function (res) {
                layer.msg(res.msg)
            })
        })
    });
    ...
    $(document).on('click', '.preview-class', function () {
        var id = $(this).attr('data-id');
        var point_type = $(this).attr('data-point-type');
        if(point_type == 3){
            var url = window.location.protocol + "//" + window.location.host + "/wap/pages/goods/index/index?id=" + id;
        }else{
            var url = window.location.protocol + "//" + window.location.host + "/wap/pointmall/point/detail?id=" + id;
        }
...
<script type="text/html" id="is_able">
    <input type="checkbox" name="is_able" value="{{d.id}}" lay-skin="switch" lay-text="上架|下架" lay-filter="is_able" {{ d.marketable == 1 ? 'checked' : '' }}>
</script>
</div>
</body>
</html>
