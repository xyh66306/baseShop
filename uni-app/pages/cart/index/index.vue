		...
		<view class="content-top">
			<view class="cell-group margin-cell-group">
				<view class="cell-item cell-item-mid right-img">
					<view class="cell-item-hd">
						<image class="cell-hd-icon" src="/static/image/homepage.png" style="width: 32rpx;height: 32rpx;"></image>
					</view>
			...
			<view class="img-list cart-list">
				<checkbox-group class="cart-checkbox" v-for="(item, index) in cartData.list" :key="index" :val="item.id" @change="checkboxChange(item.id)">
					<view class="">
						<label class="uni-list-cell uni-list-cell-pd" v-show="item.type != 12">
						...
						<view class="img-list-item">
							...
							<view class="img-list-item-r little-right">
								<view class="little-right-t">
									...
									<text class="gift" v-if="item.type == 12">[赠品]</text>{{ item.products.name }}
										</view>
									<view class="goods-price red-price" v-show="item.type != 12">￥{{ item.products.price }}</view>
								</view>
									...
									<view class="goods-buy">
										...
										<view class="goods-numbox">
											...
											<uni-number-box v-on:change="bindChange($event, item)" :min="1" :max="item.maxStock" :value="item.nums"
											 :disabled="item.type == 12" v-if="!editStatus && item.type != 12"></uni-number-box>
											<view @click="del(index, item.id)" class="click-del" v-if="editStatus && item.type != 12">
												<image class="icon" src="/static/image/delete.png" mode=""></image>
											</view>
											<view v-if="item.type == 12" class="goods-num"> x {{item.nums}}</view>
										</view>
									</view>
								</view>
							</view>
