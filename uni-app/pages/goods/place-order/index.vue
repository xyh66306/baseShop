<template>
	<form class="content" @submit="toPay" report-submit="true">
		<view class="content-top">
			<uni-segmented-control :current="type_current" :values="type_items" @clickItem="onTypeItem"
				style-type="text" active-color="#333" v-if="storeSwitch == 1"></uni-segmented-control>
			<view class="content">
				<view v-show="type_current === 0">
					...
					<view class="cell-group margin-cell-group" v-if="userShip && userShip.id" @click="showAddressList">
						<view class="cell-item add-title-item right-img cell-item-mid">
							...
							<view class="cell-item-bd cell-item-bd-block">
								...
								<view class="cell-bd-view">
									<text class="cell-bd-text address">{{ userShip.area_name || '' }}
										{{ userShip.address || '' }}</text>
								</view>
							</view>
					...
					<view class="cell-group margin-cell-group" v-else>
						<view class="cell-item add-title-items"><button class="btn btn-b" @click="goAddress()"
								hover-class="btn-hover2">添加收货地址</button></view>
					</view>
				</view>
				<view v-show="type_current === 1">
					<!-- 门店信息 -->
					<view v-if="store && store.id && store.id != 0" class="cell-group margin-cell-group"
						@click="goStorelist()">
						<view class="cell-item add-title-item right-img cell-item-mid">
							<view class="cell-item-hd">
			...
			<view class="cell-group margin-cell-group" v-if="storeSwitch == 1 && type_current === 1">
				<view class="cell-item user-head cell-item-mid">
					...
					<view class="cell-item-bd" style="width:88%;">
						<input class="cell-bd-input" style="width: 100%;" placeholder="请输入提货人姓名"
							v-model="store_pick.name" />
					</view>
				</view>
				<view class="cell-item cell-item-mid">
					...
					<view class="cell-item-bd" style="width: 88%;">
						<input class="cell-bd-input" placeholder="请输入提货人电话" v-model="store_pick.mobile"
							style="width:100%;" />
					</view>
				</view>
			</view>
			...
			<!-- 商品列表信息 -->
			<view class="img-list">
				<view class="img-list-item" v-if="item.is_select == true" v-for="(item, index) in products"
					:key="index">
					<image class="img-list-item-l little-img have-none" :src="item.products.image_path"
					...
					<view class="img-list-item-r little-right">
						<view class="little-right-t">
							<view class="goods-name list-goods-name" @click="goodsDetail(item.products.goods_id)">
								<text class="gift" v-if="item.type == 12">[赠品]</text>
								{{ item.products.name || '' }}
							</view>
							<view class="goods-price" v-if="item.type != 12">￥{{ item.products.price || '0.00' }}</view>
						</view>
						<view class="romotion-tip" v-if="item.type != 12 && item.products.promotion_list1.length">
						...
						<view class="goods-item-c">
							<view class="goods-buy">
								<view class="goods-salesvolume" v-if="item.products.spes_desc !== null">
									<text>{{ item.products.spes_desc || '' }}</text>
								</view>
							</view>
						</view>
						<view style="display: flex; justify-content: space-between;" class="goods-sales-info">
							<view v-if="orderType != 10">
								<view class="price">
							...
							<view v-else="orderType == 10">
								<view class="price" v-if="item.products.open_point == 1">
									<text class="symbol">￥</text>
									{{ item.products.price || '0.00' }}+{{ item.products.point || '0' }}积分
								</view>
								<view class="price" v-if="item.products.open_point == 2">
									{{ item.products.point || '0' }}积分
								</view>
							</view>
			...
			<view class="cell-group">
				...
				<view class="cell-item add-title-item right-img"
					v-if="isOpenPoint === 1 && userPointNums > 0 && orderType!='10'">
					<view class="cell-item-bd" style="margin-left:0 ;">
						...
						<view class="cell-bd-view" style="margin-bottom: 4rpx;">
							<text class="cell-bd-text address color-9">
								可用 {{ canUsePoint || '' }} 积分，可抵扣 {{ pointMoney || '' }} 元，共有 {{ userPointNums || '' }}
								积分。
							</text>
						</view>
					</view>
				...
				<view class="cell-item">
					<view class="cell-item-hd cell-item-bd-block">
						...
						<view class="cell-hd-view" v-if="cartData.point > 0 && orderType!=10">积分抵扣</view>
						<view class="cell-hd-view" v-if="cartData.point > 0 && orderType==10">使用积分</view>
						<view class="cell-hd-view" v-if="orderType==10">可用积分</view>
					...
					<view class="cell-item-ft cell-item-bd-block">
						...
						<view class="cell-ft-view" v-if="cartData.goods_pmt_old > 0">-{{ cartData.goods_pmt || '' }}
						</view>
						<view class="cell-ft-view" v-if="cartData.order_pmt_old > 0">-{{ cartData.order_pmt || '' }}
						</view>
						<!-- <view class="cell-ft-view" v-if="!couponIsUsed">-{{ cartData.coupon_pmt || '' }}</view> -->
						<view class="cell-ft-view" v-if="cartData.point > 0 && orderType!=10">
							-{{ cartData.point_money || '' }}</view>
						<view class="cell-ft-view" v-if="cartData.point > 0 && orderType==10">
							{{ cartData.point || '0' }}
						</view>
						<view class="cell-ft-view" v-if="orderType==10">{{ cartData.available_point || '0' }}</view>
			...
			<view style="width: 100%;height: 700rpx;background: #F8F8F8;;position: absolute;left:0;bottom: 0;">
				<view class="pop-c">
					...
					<view class="pop-b">
						<view class="pop-b-t">
							<uni-segmented-control :current="current" :values="items" @clickItem="onClickItem"
								style-type="text" active-color="#333"></uni-segmented-control>
						</view>
						<view class="" v-show="current === 0">
							<scroll-view class="coupon-c" scroll-y="true" style="" v-if="userCoupons.length">
								<view class="coupon-c-item" v-for="(item, index) in userCoupons" :key="index">
									<view :class="item.cla">
										<view class="cci-l-c color-f">coupon</view>
									</view>
										...
										<view class="cci-r-c">
											...
											<view class="ccirc-b">
												<view class="ccirc-b-l">
													<view class="ccirc-b-tip">{{ item.expression1 + item.expression2 }}
													</view>
													<view class="ccirc-b-time color-9">有效期至：{{ item.etime }}</view>
												</view>
												<view class="ccirc-b-r color-f" @click="couponHandle(index)"
													v-if="!item.checked && !item.disabled">立即使用</view>
												<view class="ccirc-b-r color-f" @click="couponHandle(index)"
													v-else-if="item.checked && !item.disabled">取消使用</view>
						...
						<view class="coupon-c" v-show="current === 1">
							<scroll-view class="coupon-c" scroll-y="true">
								<view class="coupon-enter">
									<view class="coupon-input"><input type="text" v-model="inputCouponCode"
											placeholder="请输入优惠券码" /></view>
									...
									<view class="coupon-enter-btn" @click="useInputCouponCode"><button
											class="btn btn-b">确认</button></view>
								</view>
									...
									<view class="cci-r">
										<view class="cci-r-c">
											<view class="ccirc-b" style="padding: 20rpx 10rpx;">
												<view class="ccirc-b-l">
													<view class="ccirc-b-tip one-line" style="margin-bottom: 40rpx;">
														{{ item.name || '' }}
													</view>
													<view class="ccirc-b-time color-9">{{ item.code }}</view>
												</view>
												<view class="ccirc-b-r color-f" @click="removeCouponCode1(item.code)">
													取消使用</view>
											</view>
										</view>
									</view>
					...
					<view class="button-bottom">
						<button class="btn btn-square" @click="notUseCoupon()">不使用优惠券</button>
		...
		<view class="button-bottom">
			<view class="button-bottom-c" v-if="orderType==10">
				<view class="button-bottom-c-t">共 {{ productNums || '' }} 件商品</view>
				<view class="button-bottom-c-b">
					合计
					<text class="red-price">{{ cartData.amount || '' }}+{{ cartData.point || '' }}积分</text>
				</view>
			</view>
			<view class="button-bottom-c" v-if="orderType!=10">
				<view class="button-bottom-c-t">共 {{ productNums || '' }} 件商品</view>
			...
			<button class="btn btn-square btn-b" hover-class="btn-hover2" form-type="submit" :disabled="submitStatus"
				:loading="submitStatus" v-if="orderType!=10">立即支付</button>
			<button class="btn btn-square btn-b" hover-class="btn-hover2" form-type="submit" :disabled="submitStatus"
				:loading="submitStatus" v-if="orderType==10">立即兑换</button>
		</view>
	</form>
</template>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.order_type) {
				...
				this.orderType = parseInt(options.order_type);
			}
			if (options.team_id) {
				...
				this.$api.cartList(data, res => {
					if (res.status) {
						...
						if (res.data == "15030") {
							...
							// 第一种 当前选择的优惠券权重最大
							let _this = this;
							if (this.optCoupon == res.exclusive_code || this.inputCouponCode == res
								.exclusive_code) {
								// 排斥的优惠券(权重小的)
									...
									success: function(res1) {
										if (res1.confirm) {
											...
											_this.userCoupons.forEach(item5 => {
												if (item5.coupon_code == _this.optCoupon ||
													item5.coupon_code == _this.inputCouponCode
												) {
											...
											_this.inputCouArr.forEach((item6, index1) => {
												if (item6.code == _this.optCoupon || item6
													.code == _this.inputCouponCode) {
											...
											paramsCodes.forEach((item7, index2) => {
												if (item7 == _this.optCoupon || item7 == _this
													.inputCouponCode) {
													paramsCodes.splice(index2, 1);
												}
											});
										...
										if (res2.confirm) {
											...
											_this.userCoupons.forEach(item => {
												if (_this.optCoupon == item.coupon_code) {
													item.checked1 = true;
												} else if (res.exclusive_code == item
													.coupon_code) {
													item.checked1 = false;
													item.checked = false;
												}
					...
					_this.$api.createOrder(data, res => {
							if (res.status) {
								...
								if (res.data.pay_status == '2') {
									_this.$common.redirectTo('/pages/goods/payment/result?order_id=' + res.data
										.order_id);
								} else {
									_this.$common.redirectTo('/pages/goods/payment/index?order_id=' + res.data
										.order_id + '&type=' + _this.orderType);
								}
							} else {
